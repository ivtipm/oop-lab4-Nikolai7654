...
namespace Database
{
    class WorkBD
    {
        ...
        /// </summary>
        public void ChangeType(string type, int index)
        {
            Item it = (Item)itemList[index];
            if (type =="")
        ...
        public void OpenFile(string filename)
        {
            ...
            using (StreamReader sw = new StreamReader(filename))
            {
                while (!sw.EndOfStream)
                {
                    ...
                    ushort volume = (ushort)Convert.ToInt32(dataFromFile[2]);
                    string type = (string)Convert.ToString(dataFromFile[3]);
                   
                    ushort cost = (ushort)Convert.ToInt32(dataFromFile[4]);
